...
```{r, warning = F, message = F, echo = F, fig.width = 4, fig.height = 3, fig.cap="Mean Opinion of Sam by Treatment Group"}
...
```{r, warning = F, message = F, echo = F, fig.width = 4, fig.height = 3, fig.cap="Density Plot of Opinion Variable"}
...
We now check to see if a transformation of the responses can help satisfy the condition of normality. First, we plot the mean and standard deviations for the response variable, likability of Sam, across the treatment groups.
...
```{r, warning = F, message = F, echo = F, fig.width = 4, fig.height = 3, fig.cap = "Log(mean) vs. Log(sd) Plot to Assess Possible Data Transformations"}
...
Using the means and standard deviations, we create a linear regression model and look at the slope. The slope is about -0.65 and so 1-slope is 1.65, thus, we can try to see if a reciprocals transformation can help.
...
```{r,warning = F, message = F, echo = F, fig.width = 4, fig.height = 3, fig.cap = "Density Plot of Transformed DV"}
ggplot(df, aes(x=recip_opinion, color=condition))+geom_density()
```


The distributions are still not normal so a reciprocals transformation did not help. Similarly, we look at the results of the Tukey-nonadditivity plot. 


```{r, warning = F, message = F, echo = F, fig.width = 4, fig.height = 3, fig.cap = "Tukey Non-Addtivity Plot for Opinion by Vignette Age and Behavior"}
...
```{r, warning = F, message = F, echo = F, fig.width = 4, fig.height = 3, fig.cap = "Tukey Non-Addtivity Plot for Reciprical Opinion by Vignette Age and Behavior"}
...
```{r, warning = F, message = F, echo = F, fig.width = 4, fig.height = 3, fig.cap = "Mean Opinion od Sam by Vignette Age and Behavior"}
...
The above Tukey plot has a nonzero slope, which indicates that there is evidence of non-additivity. However, the interaction plot shows that the lines for Good and Bad behavior do not intersect but have slopes of opposite signs. 
...
The Tukey HSD test shows that opinons of the behavior groups are different from one another, while the age groups are not. This supports our earlier conclusions that behavior is a significant predictor of mean opinion of the vignette character, but not stated character age. Pairwise comparisonf from the Tukey test also show that the comparisons wher behavior differs (Good vs. Bad) have a meaningful difference in means, regardless of whether or not age differs as well. 
...
# Conclusion 
  ...
  There were several limitations to this study. First, the normality condition is violated in our Anova models, so the significant results indicating that a person’s opinion of another differed depending on the other person's behavior may not be reliable. However, our Kruskal Wallis Test results later yielded significant results indicating a difference in opinion across varying behavior types. Additionally, a greater sample size can possibly serve as a solution to our normality violation. Secondly, our participants were not chosen through random sampling and do not represent the entire Smith student population. Therefore, the results of this study cannot be generalized to the entire Smith student population. Third, most of our respondents are CIS white females. As studies have shown, women tend to be more empathetic than men [@Hoffman_emp_1977] so this may influence our results.
